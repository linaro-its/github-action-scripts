...
const handleModifiers = function (flags) {
  let modifiers = ''
  if (noCaseSyntax.test(flags)) modifiers += 'i';
  if (globalSyntax.test(flags)) modifiers += 'g';
  return modifiers;
}
const parseRules = function (unparsedRules) {
  return (unparsedRules || []).map(function (rule) {
    ...
    return {
      regexp: typeof parts[2] !== 'undefined' ? new RegExp(parts[0], handleModifiers(flags)) : new RegExp(parts[0]),
      replace: parts[1],
      inverted: inverted,
