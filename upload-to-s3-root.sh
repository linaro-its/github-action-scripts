...
aws --profile "$AWS_STATIC_SITE_PROFILE" s3 sync --exclude "*" --include "*.html" --cache-control "no-cache, max-age=86400" ./ "s3://$AWS_STATIC_SITE_URL" --delete | tee "/tmp/$GITHUB_SHA.tmp"
# Sync non-HTML files with "normal" cache settings.
aws --profile "$AWS_STATIC_SITE_PROFILE" s3 sync --include "*" --exclude "*.html" --cache-control "public, max-age=86400" ./ "s3://$AWS_STATIC_SITE_URL" --delete | tee -a "/tmp/$GITHUB_SHA.tmp"
cd $DIR
